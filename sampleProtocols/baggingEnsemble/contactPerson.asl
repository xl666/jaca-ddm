...
+begin_process: true
		...
		!consut_type_count("bagger", Count);
...
+!begin: true
	<- 
		...
		+numAgents(Count);
		!waitAllReady;
		makeArtifact("baggingEnsemble","artifacts.ClassifierBaggingEnsemble",[], IdBagEn);
		!register_artifact("baggingEnsemble"); 
		//begin

		for (.range(X,1,Count))
		{
			.concat("bagger",X,NAg);
			.send(NAg, achieve, buildBag("baggingEnsemble"));
		};
		!waitAllFinish;
		.send(experimenter, tell, finish("baggingEnsemble")).
