...
+modelReceived: true //each time it receives a model
	...
	quitWorkspace;
	?instanceBaseID(IdBase);
        searchSendContradictions(J48ID, ContsFound)[artifact_id(IdBase)];
        .my_name(Name);
	.concat("/tmp/remanente", Name, Path1);
	 .concat(Path1, ".arff", Path);
	 dumpInstances2File(Path)[artifact_id(IdBase)];
