...
+modelReceived: true //each time it receives a model
	...
	quitWorkspace;
	?instanceBaseID(IdBase);
	?param("Counter examples filtering step", CounterStep);
	?param("Initial counter examples filtering", InitialCounter);
	?param("Min counters to filter", MinCounter);
	?round(Nround);
	searchSendContradictionsGPU(J48ID, InitialCounter+(CounterStep * Nround), MinCounter, ContsFound)[artifact_id(IdBase)];
	-+round(Nround+1);
	.my_name(Name);
...
+!saveLefts : true
   <-
   ?data(File);
   .concat(File, ".left.arff", FinalPath);
   dumpInstances2File(FinalPath);
   .my_name(Name); 
   .send(contactPerson1, tell, saved(Name)).
